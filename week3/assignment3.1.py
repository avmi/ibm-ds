...
def kurtosisTemperature(df,spark):    
        return spark.sql("SELECT KURTOSIS(temperature) as ktemperature from washing").first().ktemperature
...
def correlationTemperatureHardness(df,spark):
    ...
    ( 
      COVAR_POP(temperature, hardness)/
        (
            SQRT(SUM(POWER((temperature)-%s,2))/COUNT(temperature)) 
            * 
            SQRT(SUM(POWER((hardness)-%s,2))/COUNT(hardness))
         )
    )

as temperaturehardness from washing
                        """ %(meanTemperature(df,spark),meanTemperature(df,spark))).first().temperaturehardness
